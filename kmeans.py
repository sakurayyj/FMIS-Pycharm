...
def showCluster(dataSet, k, clusterAssment, centroids):
    fig = plt.figure()
    plt.title("地方财政数据聚类结果图")
    ax = fig.add_subplot(111) #表示1*1的网图，1个子图
    data = []
    ...
    plt.savefig("static/IMG/dataImg/examples7.jpg")
    plt.savefig("D:/AndroidStudioTest/FMIS/app/src/main/assets/examples7.jpg", dpi=200)
    plt.savefig("D:/AndroidStudioTest/FMIS/app/src/main/res/drawable/examples7.jpg", dpi=200)
    # plt.show()


if __name__ == "__main__":
    cluster_Num = 3
    data = loadDataSet("pima-indians-diabetes.csv")
    # data = loadDataSet("code_data/data1.csv")
    print("data:"+data)
