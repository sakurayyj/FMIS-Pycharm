...
df = pd.read_csv("code_data/data2.csv")

#删除不相关的列
df.drop(columns=["year","y"], inplace=True)

df.to_csv('code_data/newdata1.csv', index=False)

with open('code_data/newdata1.csv', 'r') as f:
    reader = csv.reader(f)
    result = list(reader)
...
dataset1=dataset[0:len(dataset)-1]
dataset2=dataset[1:len(dataset)+1]

dataset=matrix.tolist(np.multiply((np.mat(dataset2)-np.mat(dataset1)),1/np.mat(dataset1)))
...
for i in dataset:
    ...
    for j in i:
        if j>k:
            newdataset.append(flag[n])
        n+=1
    if dataset:
        newdatasets.append(newdataset)

print("增长项集为：",newdatasets)
